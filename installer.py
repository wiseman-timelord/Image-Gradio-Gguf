...
def write_default_persistent(cpu: Dict[str, Any],
                             vk: Optional[Dict[str, Any]] = None) -> None:
    """Seed persistent.json with defaults, if it does not already exist.
...
def _print_backend_banner(vk: Dict[str, Any]) -> None:
    ...
    if vk["available"]:
        ...
        print()
        print(f"     3. Compile Llama.Cpp/StableDiffusion for CPU")
        print()
        print(f"     4. Compile Llama.Cpp/StableDiffusion for Vulkan")
        print()
    else:
        print(f"     1. Download Llama.Cpp/StableDiffusion for CPU")
        print()
        print(f"     2. Compile Llama.Cpp/StableDiffusion for CPU")
        print()
    print()
    print()
    print()
...
def _run_summary(t0: float) -> None:
    ...
    log(f"persistent   : {_state(_PERSIST_PATH, 'user config')}")
...
def main() -> None:
    ...
    if args.detect_only:
        ...
        write_default_persistent(cpu, vk)
    ...
    while True:
        ...
        if choice == "1":
            ...
            write_constants(cpu, vk, use_vulkan=use_vulkan)
            # persistent.json is written LAST so its device defaults can be
            # anchored to a GPU that actually exists on this machine.
            write_default_persistent(cpu, vk)
            _run_summary(t0)
            return
        if choice == "2":
        ...
        if choice == "3":
            ...
            write_default_persistent(cpu, vk_now)
            _run_summary(t0)
            return
