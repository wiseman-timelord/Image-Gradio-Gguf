...
- While the program works ok normally after starting up the computer, when I put the computer into sleep mode, then continue using (likely I have closed the program and restarted the program, but I am not sure), but it seems to have issues, where during a late phase in processing it has an error. Possibly this is to do with the, tight space or drivers, on my dated graphics card on windows 10. Unsure. A restart of the computer seems to fix this, but it should make no difference on a passive card?! Needs further testing/diagnosis before I can attempt to fix this.
- When running inference on the passive card, it seems to be doing things on the GPU of the primary card. Possibly its storing the model on one card and processing on the other? The RX 470 (passive)  is not showing any GPU activity, and I am unsure if it is always like that, but I do NOT think so. I am surprised it shows the GPU memory at all on my RX 470, possibly it does that since installing the vulkan SDK? Or windows 10 just does that anyway? Keep in mind its NOT ROCM or CUDA.
