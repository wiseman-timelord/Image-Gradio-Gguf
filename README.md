...
- intended Image generation model "z_image_turbo-Q#.gguf" and "ae.safetensors". Such as for example, [Vanilla Z-Image-Turbo-GGUF](https://huggingface.co/unsloth/Z-Image-Turbo-GGUF) or [Reinforced Z-Image-Turbo-GGUF](https://huggingface.co/BigDannyPt/Z-Image-Turbo-GGUF-Collection/tree/main).
...
### Notation:
- Something to consider is how much memory the Image model takes, I was able to load Q5 model to passive 8GB GPU, if Diffuser Placement is set to Split, owtherwise it will not load in Full. Image models need more space when loaded.
