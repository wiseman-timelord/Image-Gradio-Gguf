...
- Current Phase v1.19-v1.21 - Image Generation Model support expanded to addditionally include [Z-Image-Turbo-GGUF-Collection](https://huggingface.co/BigDannyPt/Z-Image-Turbo-GGUF-Collection) in v1.19, this was then completed/corrected for v1.20 to include individual path setting for "ae.safetensors", this was then corrected again in v1.21 to fix to the Generate Image button not appearing, after recent changes to `ae.safetensors` handling. v1.21 also sees a new a Preferences page, which makes the Configuration page a little cleaner and more focused. 
