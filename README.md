...
- Current Phase v1.19-v1.21 - Image Generation Model support expanded to addditionally include [Z-Image-Turbo-GGUF-Collection](https://huggingface.co/BigDannyPt/Z-Image-Turbo-GGUF-Collection) in v1.19, this was then completed/corrected for v1.20 to include individual path setting for "ae.safetensors". v1.21 sees a fix to the Generate Image button, after recent changes to `ae.safetensors` handling. There is now a Preferences page, which makes the Configuration page a little cleaner. 
...
### Media
- Generation page, recently not much changed here (v1.21)...  
...
- Preferences page, recently created and minimal, but strictly program settings go here (v1.21)...  
