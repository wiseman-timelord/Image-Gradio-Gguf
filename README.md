...
- When I put the computer into sleep mode, then continue using, likely I have closed the program and restarted the program, but it seems to have issues, where during a late phase in processing it has an error. Possibly this is to do with the, tight space or drivers, on my dated graphics card on windows 10. Unsure. A restart of the computer seems to fix this, but it should make no difference on a passive card?! Needs further testing/diagnosis.
